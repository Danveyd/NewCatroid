...
class NativeLayerBrick(private var spinnerSelectionON: Int = 0) : BrickBaseType() {
    ...
    private fun assignVideoSpinnerProperties() {
        view.findViewById<Spinner>(R.id.brick_native_layer_spinner).apply {
            adapter = createArrayAdapter(context)
            onItemSelectedListener = AdapterViewOnItemSelectedListenerImpl { position ->
                spinnerSelectionON = position
            }
            setSelection(spinnerSelectionON)
        }
    }

    private fun createArrayAdapter(context: Context): ArrayAdapter<String?> {
        val spinnerValues = arrayOf(
            context.getString(R.string.nlb_in_front_of_scene),
            context.getString(R.string.nlb_behind_scene)
        )
        val spinnerAdapter =
    ...
    override fun addActionToSequence(sprite: Sprite, sequence: ScriptSequenceAction) {
        sequence.addAction(sprite.actionFactory.nativeViewLayer(spinnerSelectionON))
    }
}
